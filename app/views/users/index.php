...
<div class="top-bar"
    ...
    <a href="/PMS/user/login" class="login-btn">Login</a>
</div>
        ...
        <?php foreach ($products as $product): ?>
            <div class="card" style="width: 18rem;">
                ...
                <div class="card-body">
                    ...
                    <a href="/PMS/user/show/<?php echo $product['id']; ?>" class="btn btn-primary">View</a>
                </div>
            </div>
        <?php endforeach; ?>
    <?php else: ?>
...
<div style="text-align:center; margin:20px;">
    <?php if ($totalPage > 1): ?>
        <?php if ($page > 1): ?>
            <a href="/PMS/user/showActiveProducts?page=<?php echo $page - 1; ?>" class="btn btn-secondary">Previous</a>
        <?php endif; ?>

        <?php for ($i = 1; $i <= $totalPage; $i++): ?>
            <a href="/PMS/user/showActiveProducts?page=<?php echo $i; ?>"
        ...
        <?php if ($page < $totalPage): ?>
            <a href="/PMS/user/showActiveProducts?page=<?php echo $page + 1; ?>" class="btn btn-secondary">Next</a>
        <?php endif; ?>
    <?php endif; ?>
</div>
...
<script>
    ...
    function fetchProducts() {
        ...
        fetch(`/PMS/user/search/${encodeURIComponent(query)}/${encodeURIComponent(brand)}`)
            .then(res => res.json())
            .then(products => {
                container.innerHTML = "";
                        ...
                        <div class="card" style="width: 18rem;">
                            ...
                            <div class="card-body">
                                <div style="display:flex; align-items:center; justify-content:space-between;">
                                    ...
                                    ${product.brand_logo ?
                            `<img src="/PMS/public/uploads/${product.brand_logo}" 
                                              alt="Brand Logo" 
                                              style="width:35px; height:35px; object-fit:contain; margin-left:8px;">`
                            : ''
                        }
                                ...
                                <a href="/PMS/user/show/${product.id}" class="btn btn-primary">View</a>
                            </div>
                        </div>`;
                });
            })
